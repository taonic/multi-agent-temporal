...
def create_tool_from_mixed_items(*items, sub_agents: Dict[str, type] = None) -> Tool:
    """
    Convenience function to create a Tool from a mix of functions, dataclasses, and sub-agents.
    
    Args:
        *items: Variable number of functions and/or dataclass types
        sub_agents: Optional dictionary mapping agent names to their dataclass schemas
        
    Returns:
        Tool object with all items converted to function declarations
    """
    return create_enhanced_tool(list(items), sub_agents=sub_agents)

def create_sub_agent_tool(sub_agents: Dict[str, type]) -> Tool:
    """
    Create a Tool specifically from a dictionary of sub-agents.
    
    Args:
        sub_agents: Dictionary mapping agent names to their dataclass schemas
        
    Returns:
        Tool object with sub-agent function declarations
        
    Raises:
        ValueError: If any schema is not a dataclass
    """
    function_declarations = []
    ...
    for agent_name, schema_class in sub_agents.items():
        if not is_dataclass(schema_class):
            raise ValueError(
                f"Sub-agent '{agent_name}' schema must be a dataclass, "
                f"but got {type(schema_class)}: {schema_class}"
            )
        ...
        try:
            ...
            raise ValueError(f"Failed to create declaration for sub-agent {agent_name}: {e}")
